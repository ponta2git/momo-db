import { check, date, index, integer, jsonb, pgTable, primaryKey, text, timestamp, uniqueIndex, varchar } from "drizzle-orm/pg-core";
...
export const sessions = pgTable("sessions", {
    ...
    uniqueIndex("sessions_week_key_postpone_count_unique").on(table.weekKey, table.postponeCount),
    // invariant: status を SESSION_STATUSES と DB CHECK で二重ガードし drizzle 型との乖離を防ぐ。
    check("sessions_status_check", sql `${table.status} IN ('ASKING','POSTPONE_VOTING','POSTPONED','DECIDED','CANCELLED','COMPLETED','SKIPPED')`),
    // invariant: 順延は 1 回まで (金 → 土)。 @see requirements/base.md §4
    check("sessions_postpone_count_check", sql `${table.postponeCount} IN (0, 1)`),
    ...
    index("idx_sessions_status_reminder").on(table.status, table.reminderSentAt, table.reminderAt)
]);
export const responses = pgTable("responses", {
    ...
    uniqueIndex("responses_session_member_unique").on(table.sessionId, table.memberId),
    // invariant: choice を RESPONSE_CHOICES と DB CHECK で二重ガード。
    check("responses_choice_check", sql `${table.choice} IN ('T2200','T2230','T2300','T2330','ABSENT','POSTPONE_OK','POSTPONE_NG')`)
]);
...
export const heldEventParticipants = pgTable("held_event_participants", {
...
export const OUTBOX_KINDS = ["send_message", "edit_message"];
...
export const discordOutbox = pgTable("discord_outbox", {
    ...
    check("discord_outbox_kind_check", sql `${table.kind} IN ('send_message','edit_message')`),
    check("discord_outbox_status_check", sql `${table.status} IN ('PENDING','IN_FLIGHT','DELIVERED','FAILED')`),
    ...
    index("idx_discord_outbox_status_next").on(table.status, table.nextAttemptAt)
]);
