...
作品・シーズン通算は全マップを含む。シーズン一覧は追加・変更・削除で影響するシーズンだけとし、削除前とシーズン移動前後の所属を含める。手動・保守目的だけの再計算は通知しない。実際の試合変更を含む成功だけを通知対象とし、入力差分がなければ送らない。掲載試合は前回成功から追加・更新された対象で、全履歴を表示時に再検索しない。削除試合は掲載せず、対象試合なしを全員銀次 0 回と区別する。未入力メモ欄は省略する。後日のメモ・表示名・試合編集を本文へ反映しない。リンク先は現在の下書き・試合詳細と最新分析であり、通知時点の分析を固定して表示する画面ではない。
...
`series_analysis_title_states.notification_baseline_state` は `initial` / `artifact` / `unknown`。
`artifact` だけが同一作品の `notification_baseline_artifact_id` を持ち、DB は形状と参照整合性を保証する。
これは画面用の current / previous pointer と独立した最後の成功入力であり、世代変更や画面 pointer の解除で消さない。
参照中の成果物は履歴整理から除外する。作品削除時は業務 command が参照を解放してから削除する。

default は `unknown`。新規作品を作成するアプリ command は、同一 transaction で `initial` を明示する。
0054 はこの保存形を追加し、0055 は既存 current pointer が同一作品の公開済み・検証済み成果物を指す場合だけ
`artifact` を初期化する。成果物がない作品も含め、それ以外は `unknown` のままとし、空であることから初回を推測しない。

producer は成功確定と同じ transaction で基準を進める。通知設定 OFF、手動・保守目的、入力差分なしの場合も進め、
後の成功で古い変更を再通知しない。`initial` では登録済み全試合を初回として扱う。
`unknown` や参照内容を読み出せない場合は初回に読み替えず、通知を省略して理由を記録する。
削除だけの差分は削除試合を掲載せず、影響する作品・シーズンの集計だけを含める。
入力の比較と通知対象の判断は Worker、読めない旧形式の集計を比較不能とする判断もアプリに置く。
